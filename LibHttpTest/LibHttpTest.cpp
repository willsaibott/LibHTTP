...
namespace LibHttpTest
{
                       ...
                       boost::string_view target)
    {
      ...
      Assert::AreNotEqual(0ull, http::read(stream, buffer, res));
    ...
    TEST_METHOD(TestHttpServerBind) {
      ...
      Assert::IsFalse(port_in_use(18080));
      ...
      Assert::IsTrue(port_in_use(18080));

      {
        auto response = do_http_request_to(interface_address, "18080", "/");

        auto bufs = response.body().data();
        std::string actual(bufs.begin(), bufs.end());
      ...
      {
        auto response = do_http_request_to(interface_address, "18080", "/print");

        std::string actual;
        actual.reserve(boost::asio::buffer_size(response.body()));
        for (boost::asio::const_buffer b : response.body().cdata()) {
          actual.append(boost::asio::buffer_cast<char const*>(b),
    ...
    TEST_METHOD(TestWebSocketServerBind) {
      ...
      Assert::IsFalse(port_in_use(18090));
      std::thread thread{
        [&websocket_server]() {
      ...
      std::this_thread::sleep_for(std::chrono::seconds{ 5 });
      Assert::IsTrue(port_in_use(18090));
    ...
    TEST_METHOD(TestHttpsServerBind) {
        ...
        Assert::IsFalse(port_in_use(18045));
        std::thread thread{
          [&https_server]() {
        ...
        Assert::IsTrue(port_in_use(18045));
        {
          auto response = do_http_request_to(interface_address, "18045", "/");
        ...
        {
          auto response = do_http_request_to(interface_address, "18045", "/print");
    ...
    TEST_METHOD(TestSecureWebSocketServerBind) {
       ...
       Assert::IsFalse(port_in_use(18095));
       std::thread thread{
         [&websocket_ssl_server]() {
           websocket_ssl_server.start(std::thread::hardware_concurrency());
         }
       };
       std::cout << "Websocket SSL server running at wss://" << interface_address << ":18095..." << std::endl;
       std::this_thread::sleep_for(std::chrono::seconds{ 5 });
